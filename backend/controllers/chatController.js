import asyncHandler from 'express-async-handler'
import ChatModel from './../models/ChatModel.js'
import UserModel from './../models/UserModel.js'

class ChatController{
    static accessChat = asyncHandler(async (req, res) => {
        const {userId} = req.body;
        if(!userId){
            return res.status(400).send({message: "UserID param not sent with request"});
        }

        let isChat = await ChatModel.find({
            isGroupChat: false,
            $and: [
                {users: {$elemMatch: {$eq: req.user._id}}},
                {users: {$elemMatch: {$eq: userId}}},
            ]
        }).populate("users", "-password").populate("latestMessage");
        isChat = await UserModel.populate(isChat, {
            path: "latestMessage.sender",
            select: "name pic email",
        });
        if(isChat.length>0){
            res.send(isChat[0]);
        }
        else{
            let chatData = {
                chatName: "sender",
                isGroupChat: false,
                users: [req.user._id, userId],
            }
            try {
                const createdChat = await ChatModel.create(chatData);
                const fullChat = await ChatModel.findOne({_id: createdChat._id}).populate("users", "-password")
                res.status(200).send(fullChat)
            } catch (error) {
                res.status(400);
                throw new Error(error.message);
            }
        }
    });

    static fetchChat = asyncHandler(async (req, res) => {
        try {
            let results = await ChatModel.find({users: {$elemMatch: {$eq: req.user._id}}})
            .populate("users", "-password")
            .populate("groupAdmins", "-password")
            .populate("latestMessage")
            .sort({updatedAt: -1})
            results = await UserModel.populate(results, {
                path: "latestMessage.sender",
                select: "name pic email",
            })
            res.status(200).send(results);
        } catch (error) {
            res.status(400);
            throw new Error(error.message);
        }
    });

    static createGroupChat = asyncHandler(async (req, res) => {
        if(!req.body.users || !req.body.name){
            return res.status(400).send({message: "Please fill all the fields"});
        }
        let users = JSON.parse(req.body.users);
        if(users.length<2){
            return res.status(400).send({message: "More than 2 users are required to form a group chat"});
        }
        users.push(req.user);
        try {
            const groupChat = await ChatModel.create({
                chatName: req.body.name,
                users: users,
                isGroupChat: true,
                groupAdmins: [req.user],
            })
            const fullGroupChat = await ChatModel.findOne({_id:groupChat._id})
            .populate("users", "-password")
            .populate("groupAdmins", "-password");
            res.status(200).json(fullGroupChat);
        } catch (error) {
            res.status(400);
            throw new Error(error.message);
        }
    });

    static updateGroup = asyncHandler(async (req, res) => {
        const {chatId, chatPic, chatName, chatDesc} = req.body;
        if(!chatId){
            res.status(400).send({message: 'chatId not found'});
        }
        if(!chatPic && !chatName && !chatDesc){
            res.status(400).send({message: 'Please fill all the fields'});
        }
        const updateFields = {};
        if (chatPic) {
            updateFields.chatPic = chatPic;
        }
        if (chatName) {
            updateFields.chatName = chatName;
        }
        if (chatDesc) {
            updateFields.chatDesc = chatDesc;
        }
        const updatedChat = await ChatModel.findByIdAndUpdate(chatId, updateFields,
        {
            new: true,
        })
        .populate("users", "-password")
        .populate("groupAdmins", "-password");
        if(!updatedChat){
            res.status(400);
            throw new Error("Chat not found");
        }
        else{
            res.json(updatedChat);
        }
    });

    static addToGroup = asyncHandler(async (req, res) => {
        const {chatId, userId} = req.body;
        if(!chatId || !userId){
            res.status(400).send({message: 'chatId or userId not sent with request'})
        }
        const added = await ChatModel.findByIdAndUpdate(chatId,{
            $push: {users: userId},
        },
        {
            new: true,
        })
        .populate("users", "-password")
        .populate("groupAdmins", "-password");
        if(!added){
            res.status(400);
            throw new Error("Chat not found");
        }
        else{
            res.json(added);
        }
    });

    static removeFromGroup = asyncHandler(async (req, res) => {
        const {chatId, userId} = req.body;
        if(!chatId || !userId){
            res.status(400).send({message: 'chatId or userId not sent with request'})
        }
        const removed = await ChatModel.findByIdAndUpdate(chatId,{
            $pull: {users: userId},
        },
        {
            new: true,
        })
        .populate("users", "-password")
        .populate("groupAdmins", "-password");
        if(!removed){
            res.status(400);
            throw new Error("Chat not found");
        }
        else{
            res.json(removed);
        }
    });

    static addGroupAdmin = asyncHandler(async (req, res) => {
        const {chatId, userId} = req.body;
        if(!chatId || !userId){
            res.status(400).send({message: 'chatId or userId not sent with request'})
        }
        const added = await ChatModel.findByIdAndUpdate(chatId,{
            $push: {groupAdmins: userId},
        },
        {
            new: true,
        })
        .populate("users", "-password")
        .populate("groupAdmins", "-password");
        if(!added){
            res.status(400);
            throw new Error("Chat not found");
        }
        else{
            res.json(added);
        }
    })

    static removeGroupAdmin = asyncHandler(async (req, res) => {
        const {chatId, userId} = req.body;
        if(!chatId || !userId){
            res.status(400).send({message: 'chatId or userId not sent with request'})
        }
        const removed = await ChatModel.findByIdAndUpdate(chatId,{
            $pull: {groupAdmins: userId},
        },
        {
            new: true,
        })
        .populate("users", "-password")
        .populate("groupAdmins", "-password");
        if(!removed){
            res.status(400);
            throw new Error("Chat not found");
        }
        else{
            res.json(removed);
        }
    })
}

export default ChatController;